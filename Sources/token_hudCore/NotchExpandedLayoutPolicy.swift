...
enum NotchExpandedLayoutPolicy {
    ...
    static let modelCardEstimatedColumns = 2
    ...
    static func layout(
        ...
        case .adaptive:
            ...
            let cardRows = max(
                modelCardRows(for: cardCount),
                modelContentRows(for: widgetCount)
            )
            let cardSpacing = CGFloat(max(0, cardRows - 1)) * modelCardSpacing
            let idealHeight = max(
                minimumBodyHeight,
                CGFloat(cardRows) * modelCardHeight + cardSpacing + modelCardVerticalPadding
            )
    ...
    private static func modelContentRows(for widgetCount: Int) -> Int {
        let count = max(1, widgetCount)
        return Int(ceil(Double(count) / 3.0))
    }
}
